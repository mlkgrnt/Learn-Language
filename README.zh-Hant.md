...
| 功能 | v1 | v2 |
|------|----|----|
| 狀態檔案 | `progress.json` + `course.json` | 統一為 `state.json` |
| 複習系統 | 無 | 間隔重複（掌握度評分、自動排期複習） |
| 練習模式 | 無 | `/practise` —— 自由對話 + 即時糾錯 |
| 複習課 | 無 | 每5堂一次複習課 |
| 回到課程 | 重新走一遍設定流程 | 自動續課，顯示進度卡片 |
| 課程模板 | 固定格式 | 範例驅動，靈活 |
| 詞彙預熱 | 無 | 學新詞前先複習舊詞 |
...
| 指令 | 功能 |
|------|------|
| `/process-material` | 匯入教材（PDF、Word、Excel 等），轉換為結構化資料 |
| `/learn-language` | 互動式上課，含練習、間隔重複、自適應節奏 |
| `/practise` | 目標語言自由對話，即時糾錯 |
...
學習機會：
1. 偵測已有進度（`state.json`）—— 有的話自動續課
2. 新課程：分析教材、研究學時、生成課程序列
3. 互動式上課，含練習和回饋
4. 間隔重複 —— 詞彙和語法點自動排期複習
5. 每5堂一次複習課
6. 每堂課結束自動儲存進度
...
每種提取類型嚴格遵循模板：明確的提取規則、CEFR 等級估算、交叉引用、品質檢查清單。
...
課型在序列中交替排列：
- **詞彙課**：5-10 個新詞，含發音、例句、預熱複習舊詞
- **語法課**：規則講解、句型、例句、常見錯誤
- **閱讀課**：文章 + 理解題
- **文化課**：文化主題、真實材料、場景角色扮演
- **寫作課**：範文分析、引導寫作、結構化回饋
- **複習課**（每5堂）：間隔重複掌握度低於閾值的詞彙和語法

### 間隔重複

每個詞彙和語法點都有：
- **掌握度評分**：從 0 開始，答對上升，答錯下降
- **下次複習日期**：根據表現自動排期
- **複習次數**：追蹤複習了多少輪

掌握度低於 80% 的詞彙會自動進入預熱環節和複習課。

### 進度追蹤

所有狀態儲存在單一的 `state.json` 中：
- 課程計畫（總課數、序列、目前位置）
- 詞彙及掌握度評分和複習日期
- 語法點及掌握度評分和複習日期
- 薄弱領域和學習歷史
...
學時估算基於劍橋英語研究資料，並根據語言組合難度、已匯入教材涵蓋範圍和使用者目前水平調整。
